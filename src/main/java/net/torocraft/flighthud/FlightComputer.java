...
public class FlightComputer {
  ...
  private Float computeElytraHealth(Minecraft client) {
    ...
    if (stack != null && stack.is(Items.ELYTRA)) {
      float remain = ((float) stack.getMaxDamage() - (float) stack.getDamageValue())
  ...
  private float computeRoll(Minecraft client, float partial) {
    ...
    if (horizontalFacing2 > 0.0D && horizontalSpeed2 > 0.0D) {
      double dot =
          (velocity.x * facing.x + velocity.z * facing.z) / Math.sqrt(horizontalFacing2 * horizontalSpeed2);
      dot = Mth.clamp(dot, -1, 1);
      double direction = Math.signum(velocity.x * facing.z - velocity.z * facing.x);
      rollAngle = (float) (Math.atan(Math.sqrt(horizontalSpeed2) * Math.acos(dot) * wingPower)
          * direction * 57.29577951308);
    }
  ...
  private float computePitch(Minecraft client, float parital) {
    return client.player.getXRot(parital) * -1;
  }
  ...
  public BlockPos findGround(Minecraft client) {
    ...
    while (pos.getY() >= client.level.getMinY()) {
      pos = pos.below();
  ...
  private Float computeDistanceFromGround(float altitude, Integer groundLevel) {
    if (groundLevel == null) {
      return null;
    }
  ...
  private float computeAltitude(Minecraft client) {
    return (float) client.player.getY() - 1;
  }

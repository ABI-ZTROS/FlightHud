...
public class SpeedIndicator extends HudComponent {
  ...
  public void render(GuiGraphicsExtractor g, float partial, Minecraft mc) {
    ...
    if (CONFIG.speed_showReadout) {
      drawRightAlignedFont(mc, g, String.format("%.2f", computer.speed), xSpeedText, dim.yMid - 3);
      drawBox(g, xSpeedText - 29.5f, dim.yMid - 4.5f, 30, 10);
    }
    ...
    if (CONFIG.speed_showScale) {
      for (float i = 0; i <= 100; i = i + 0.25f) {
        ...
        if (i % 1 == 0) {
          drawHorizontalLine(g, left - 2, right, y);
          if (!CONFIG.speed_showReadout || y > dim.yMid + 7 || y < dim.yMid - 7) {
            drawRightAlignedFont(mc, g, String.format("%.0f", i), xSpeedText, y - 3);
          }
        }
